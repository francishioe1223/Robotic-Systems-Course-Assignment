...
void loop()
{
  ...
  LeftCollide_thres = LeftBump_Min + (LeftBump_Max - LeftBump_Min) * 0.95;
  RightCollide_thres = RightBump_Min + (RightBump_Max - RightBump_Min) * 0.95;
  switch (state)
  {
  case STATE_CALIBRATE:
  
    if (millis() - CalibrateTime > 6500) // 3s for calibrate bump sensor
    {
      left_pid.reset();
      right_pid.reset();
      PID_Turning = true;
...
//      state = STATE_STOP;
    ...
    if (millis() - TaskTime > 12000) // 10s box pushing Task
    {
      setBeep(200);
      state = STATE_STOP;
    }
